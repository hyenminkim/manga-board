...
//시큐리티가 /member/login (POST) 주소요청이 오면 낚아채서 로그인을 진행시킨다.
...
//member 정보 객체는 ==> memberDetails 타입 객체이어야 한다.

//따라서 로그인한 member 정보를 꺼내려면
//Security Session 에서
//   => Authentication 객체를 꺼내고, 그 안에서
//        => memberDetails 정보를 꺼내면 된다.
public class PrincipalDetails implements UserDetails {
    ...
    public PrincipalDetails(Member member){
        System.out.println("memberDetails(member) 생성: " + member);
        this.member = member;
    }

    // 해당 member 의 '권한(들)'을 리턴
    ...
    public Collection<? extends GrantedAuthority> getAuthorities() {
        ...
        List<Authority> list = memberService.selectAuthoritiesById(member.getId());   // DB 에서 member 의 권한(들)을 읽어오기
